...
from sparse.funcs import redistribution_funcs, growth_funcs, prune_funcs
...
class Masking(object):
    ...
    """
    def __init__(self, device,optimizer, prune_rate_decay, prune_rate=0.5, sparsity=0.0, prune_mode='magnitude',
    ...
    def add_module(self, modules):
        ...
        if self.noembed:
            ...
            self.remove_weight_partial_name('embed')
        ...
        print ("mask isssssssssssssssssssssssssssssssssssssssssssssssssssssssss====")
    ...
    def print_status(self):
        ...
        for module in self.modules:
            for name, weight in self.masks.items():
                ...
                layer_density = sparse_weight_num / dense_weight_num
                # print(f'sparsity of layer {name} with tensor {weight.size()} is {1-layer_density}')
        print('Final sparsity level of mask {0}: {1}'.format(self.sparsity, 1 - sparse_size / total_size))
    ...
    def init(self, model, train_loader , device, mode='snip', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'one_shot_customer':
            print('initialize by one_shot_smallgm')
            self.baseline_nonzero = 0
            weight_abs = []
            for module in self.modules:
                for name, weight in module.named_parameters():
            ...
            # Gather all scores in a single vector and normalise
            all_scores = torch.cat([torch.flatten(x) for x in weight_abs])
            num_params_to_keep = int(len(all_scores) * 0.01)

            threshold, value = torch.topk(all_scores, num_params_to_keep, sorted=True)
            acceptable_score = threshold[-1]
            ...
            value_mean=torch.mean(threshold)

            print ("value_mean",value_mean)


         
            value_mean=value_mean*0.05
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    if name not in self.masks: continue
                    self.masks[name] = ((torch.abs(weight)) < value_mean).float().data.to(device)




        if mode == 'one_shot_smallgm':
            print('initialize by one_shot_smallgm')
            self.baseline_nonzero = 0
            weight_abs = []
            for module in self.modules:
            ...
            # Gather all scores in a single vector and normalise
            all_scores = torch.cat([torch.flatten(x) for x in weight_abs])
            num_params_to_keep = int(len(all_scores) * (1-density))
        ...
        if mode == 'uniform_weight':
            print('initialized with uniform_weight')
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    total_num_nonzoros=weight.numel()
                    all_scores = torch.flatten(torch.abs(weight))
                    num_params_to_keep = int(total_num_nonzoros * density)
            ...
            for name, tensor in module.named_parameters():
                if name in self.masks:
                    if not self.half:
                        ...
                        # print("self.masks.device",self.masks[name].device)

                        # print("tensor.data",tensor.data.device)
    ...
    def hard_copy_module(self):
        ## make hard copy of module
        
        self.copy_module = copy.deepcopy(self.modules[0].state_dict())
            ...
            for name, tensor in module.named_parameters():
                if name in self.masks:
                    if not self.half:
                        ...
                        tensor.data = tensor.data*self.masks[name] + self.copy_module[name].data*(1-self.masks[name])
                        # if 'momentum_buffer' in self.optimizer.state[tensor]:
                        #     self.optimizer.state[tensor]['momentum_buffer'] = self.optimizer.state[tensor]['momentum_buffer']*self.masks[name]
                    else:
                        tensor.data = tensor.data*self.masks[name].half()
