...
def parse_args():
    ...
    ##### sparse
    parser.add_argument(
        "--noembed", action="store_true", help="noembed"
    )


    parser.add_argument(
        "--fix", action="store_true", help="fix"
    )
...
def main():
    ...
    if args.task_name is not None:
        metric = evaluate.load("glue", args.task_name,experiment_id=str(args.seed)+str(args.sparsity))
    else:
    ...
    dense_model = AutoModelForSequenceClassification.from_pretrained(
        args.model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        ignore_mismatched_sizes=args.ignore_mismatched_sizes,
    ...
    dense_model.load_state_dict(torch.load(dense_path))
    ...
    sparse_model = AutoModelForSequenceClassification.from_pretrained(
        args.model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        ignore_mismatched_sizes=args.ignore_mismatched_sizes,
    )
    sparse_model.load_state_dict(torch.load(sparse_path))
    ...
    def liner_inter(use_LTs_solutions,new_indx,eval_dataloader):
        ...
        for i in range(len(use_LTs_solutions)-1):  



            model_pre = use_LTs_solutions[i]
            model_current=  use_LTs_solutions[i+1]
            ...
            inter_num=10
            inter_value_list=[i/inter_num for i in range(inter_num+1) ]
    ...
    LTs_solutions=[dense_model,sparse_model]
    
    new_indx=np.array(range(len(LTs_solutions)))
    all_acc,all_loss=liner_inter(LTs_solutions,new_indx,eval_dataloader)
    
    
    ##### plot #####
    print ("plot =========================")

    # plt_dir= "./Weight_distribution_figs/"+str(taskname)+"/"+str(args.sparsity)
    # # plt_dir= "./Weight_distribution_figs/Customer"
    ...
    # plt.savefig(os.path.join(plt_dir, str(taskname)+".png"))
